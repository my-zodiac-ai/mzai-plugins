...
description: >
  ...
  Browser (PageViewTiming, appId=${NEW_RELIC_BROWSER_APP_ID}), segment by OS/browser (userAgentOS/userAgentName)
...
| Tool | Source | Purpose |
|------|--------|---------|
| `mcp__posthog__query-trends` | PostHog | Trend queries for CWV custom events |
| `mcp__posthog__query-run` | PostHog | Raw HogQL queries for flexible analysis |
| `mcp__posthog__insight-query` | PostHog | Run insight queries with breakdowns |
...
SELECT
  ...
  count(*) as samples
FROM PageViewTiming
WHERE appId = ${NEW_RELIC_BROWSER_APP_ID}
SINCE 7 days ago
