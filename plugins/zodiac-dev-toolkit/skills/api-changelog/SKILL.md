...
name: api-changelog
description: >
  Generate API changelogs from git diffs in NestJS controllers and DTOs.
...
in NestJS controllers and DTOs. The changelog helps mobile clients, external integrators,
...
- `GET /v2/cosmic-weather/daily` — New endpoint for daily cosmic weather forecast.
  Returns `CosmicWeatherDailyResponseDto`.
- `POST /v1/charts/compare` — New synastry comparison endpoint.
- Field `timezone` added to `CreateChartDto` (optional, `string`).
...
- `GET /v1/horoscope/daily` — Response now includes `luckyNumbers` array.

## Deprecated

- `GET /v1/horoscope/simple` — Use `GET /v2/horoscope/daily` instead.
...
| [2025-03-01](./2025-03-01-cosmic-weather-v3.md) | Cosmic Weather v3 endpoints | No |
```
...
- **Only internal changes**: If controllers changed but only in internal logic (no
  ...
  API surface impact."
- **Monorepo context**: Only look at files under `back/src/` — ignore frontend changes.
