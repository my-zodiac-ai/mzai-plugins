...
name: sync-propose
description: >
  Generate resolution proposals for drift items with rationale and confidence.
  Use when the user asks to "sync propose", "propose resolutions", or wants
  suggested fixes for spec-code drift.
---

# Spec Sync: Propose Resolutions Skill

Generate resolution proposals for each drift item. Analyzes each divergence and proposes either a spec update (backfill) or code change (align).
...
Run sync-analyze first to generate the drift report.

## Workflow

### 1. Load Drift Report

Read the drift report from the previous analysis. If it doesn't exist, instruct the user to run sync-analyze first.
...
- `--interactive`: Present each proposal for immediate approval
- `--strategy backfill-all`: Recommend BACKFILL for all conflicts
- `--strategy align-all`: Recommend ALIGN for all conflicts