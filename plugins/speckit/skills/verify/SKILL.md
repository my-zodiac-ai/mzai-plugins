...
name: verify
description: >
  Perform post-implementation verification validating code against spec.md,
  plan.md, tasks.md, and constitution.md. Use when the user asks to "verify
  implementation", "post-implementation check", "проверь реализацию", or after
...
### 4a. Parallel Verification Strategy (My Zodiac AI)

For features touching both `back/` and `front/`, split verification into **two parallel agents** via the `Agent` tool:

```
Agent 1 (Backend Verifier):
  - Scope: REVIEW_FILES filtered to back/src/**
  - Checks: A (task completion for backend tasks), B (file existence),
    C (requirement coverage for API/service/event requirements),
    E (spec intent for backend), F (constitution: EDA, DDD, adapters),
    G (NestJS module structure, Mongoose schemas, event contracts)
  - Extra: Verify events are emitted after DB commit, no forwardRef usage,
    adapter tokens for cross-module deps

Agent 2 (Frontend Verifier):
  - Scope: REVIEW_FILES filtered to front/src/**
  - Checks: A (task completion for frontend tasks), B (file existence),
    C (requirement coverage for UI/UX requirements),
    D (test coverage for Vue components), E (spec intent for UI),
    F (constitution: FSD, shared design system),
    G (FSD layer compliance, barrel exports, i18n coverage for 10 languages)
  - Extra: Verify Cosmic Glass design tokens, dark mode support,
    Capacitor compatibility, no cross-feature imports
```

After both agents return, **merge their findings** into a single verification report, deduplicating any overlapping issues.

For features touching only one side (backend-only or frontend-only), run a single agent — no parallelization needed.
...
- Provide explicit command suggestions: e.g., "Run the implement skill to address findings and re-run verification", "Implementation verified — ready for review or merge"
