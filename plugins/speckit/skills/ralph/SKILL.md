...
name: ralph
description: >
  Launch the ralph autonomous implementation loop for automated task execution.
  Use when the user asks to "ralph run", "autonomous loop", "запусти ральфа",
  or wants automated implementation with minimal interaction.
---

# Ralph Autonomous Implementation Loop Skill
...
Launch the autonomous implementation loop with configuration resolution and prerequisite validation.
...
| `tasks.md` exists | Search `specs/*/tasks.md` for current feature | Print error, suggest running tasks, STOP |
...
- `SPECKIT_RALPH_MODEL`
- `SPECKIT_RALPH_MAX_ITERATIONS`

CLI arguments from step 1 override everything.

### 5. Launch Orchestrator Script in Visible Terminal
...
Print a summary of what was launched:
- Feature name
- Model
- Max iterations
...
- This command uses lightweight setup work only
