...
name: clarify
description: >
  Identify underspecified areas in a feature spec by asking targeted clarification
  questions. Use when the user asks to "clarify spec", "clarify requirements",
  "уточни спеку", or wants to reduce ambiguity before planning.
---
...
1. Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run the specify skill or verify feature branch environment.
...
5. Integration after EACH accepted answer (incremental update approach):
    ...
    - Then immediately apply the clarification to the most appropriate section(s):
       ...
       - Data shape / entities → Update Data Model preserving ordering; note added constraints succinctly.
...
8. Report completion (after questioning loop ends or early termination):
   ...
   - If any Outstanding or Deferred remain, recommend whether to proceed to the plan skill or run clarify again.
