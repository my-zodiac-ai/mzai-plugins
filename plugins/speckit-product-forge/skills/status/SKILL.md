...
name: status
description: >
  Show the current Product Forge lifecycle status for a feature. Reads
  .forge-status.yml and reports completed/in-progress/pending phases with the
  recommended next action. Use: "forge status", "/speckit.product-forge.status"
---
